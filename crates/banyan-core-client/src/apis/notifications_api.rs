...
pub async fn delete_notification(configuration: &configuration::Configuration, notification_id: &str) -> Result<(), Error<DeleteNotificationError>> {
...
pub async fn get_all_notifications(configuration: &configuration::Configuration, ) -> Result<Vec<models::ApiNotification>, Error<GetAllNotificationsError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetAllNotificationsError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
