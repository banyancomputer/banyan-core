...
import { CharSize, ExportKeyFormat } from './types';
 ...
 */
export async function fingerprintEcPublicKey(publicKey) {
    const publicKeyBytes = await webcrypto.subtle.exportKey(ExportKeyFormat.RAW, publicKey).then((key) => new Uint8Array(key));
...
export function prettyFingerprint(buf) {
