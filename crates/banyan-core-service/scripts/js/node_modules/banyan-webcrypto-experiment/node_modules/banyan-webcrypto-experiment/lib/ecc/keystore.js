...
export default class ECCKeyStore extends KeyStoreBase {
    ...
    async fingerprintPublicExchangeKey() {
        const keyPair = await this.getExchangeKeyPair();
        return ecc.fingerprintPublicKey(keyPair.publicKey);
    }
    async fingerprintPublicWriteKey() {
        const keyPair = await this.getWriteKeyPair();
        return ecc.fingerprintPublicKey(keyPair.publicKey);
    }
    // Key Escrow and Recovery
    async exportEscrowedExchangeKeyPair(cfg) {
        const mergedCfg = config.merge(this.cfg, cfg);
        const keyPair = await this.getExchangeKeyPair();
        const escrowKey = await this.getSymmKey(mergedCfg.escrowKeyName);
        return await ecc.exportEscrowedKeyPair(keyPair.publicKey, keyPair.privateKey, escrowKey);
    }
    async exportEscrowedWriteKeyPair(cfg) {
        const mergedCfg = config.merge(this.cfg, cfg);
        const keyPair = await this.getWriteKeyPair();
        const escrowKey = await this.getSymmKey(mergedCfg.escrowKeyName);
        return await ecc.exportEscrowedKeyPair(keyPair.publicKey, keyPair.privateKey, escrowKey);
    }
    async importEscrowedExchangeKeyPair(escrowKeyPair, cfg) {
        const mergedCfg = config.merge(this.cfg, cfg);
        const escrowKey = await this.getSymmKey(mergedCfg.escrowKeyName);
        await IDB.createIfDoesNotExist(this.cfg.exchangeKeyPairName, () => ecc.importEscrowedKeyPair(escrowKeyPair, escrowKey, mergedCfg.curve, KeyUse.Exchange), this.store);
    }
    async importEscrowedWriteKeyPair(escrowKeyPair, cfg) {
        const mergedCfg = config.merge(this.cfg, cfg);
        const escrowKey = await this.getSymmKey(mergedCfg.escrowKeyName);
        await IDB.createIfDoesNotExist(this.cfg.writeKeyPairName, () => ecc.importEscrowedKeyPair(escrowKeyPair, escrowKey, mergedCfg.curve, KeyUse.Write), this.store);
    }
    // Key Operations
    async sign(msg, cfg) {
        const mergedCfg = config.merge(this.cfg, cfg);
