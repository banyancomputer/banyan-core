...
export const TombProvider = ({ children }: { children: ReactNode }) => {
	...
	const getBuckets = async () => {
		...
		for (let bucket of wasm_buckets) {
			...
			let locked = mount.locked();
			let isSnapshotValid = mount.hasSnapshot();
			buckets.push({
				mount: mount || null,
				id: bucket.id(),
				name: bucket.name(),
	...
	const deleteFile = async (bucket: Bucket, path: string[], name: string) => {
		...
		const isSnapshotValid = mount.hasSnapshot();
		updateBucketsState('isSnapshotValid', isSnapshotValid, bucket.id);
		await updateStorageUsageState();
	};
