...
export class ToastNotifications {
	static notify(message: string, icon?: ReactElement) {
		toast.info(message, {
			theme: 'light',
	...
	static error(message: string, buttonMessage: string, callback: () => void) {
		toast.error(
			<div className="w-full flex flex-col items-start justify-between gap-2 text-xs">
				{message}
				<button className="text-text-600" onClick={callback}>
					{buttonMessage}
				</button>
			</div>,
			{
				icon: (
					<span className="bg-mainBackground p-2 rounded-full">
						<Close />
					</span>
				),
				hideProgressBar: true,
