...
export async function fingerprintDeviceApiPublicKeyPem(
	...
	const publicKey = await importPublicKeyPem(pem, EccCurve.P_384, KeyUse.Write);
...
export async function fingerprintEcPublicKey(
	...
	const publicKeyBytes = await webcrypto.subtle
		.exportKey(ExportKeyFormat.RAW, publicKey)
