...
export default class ECCKeyStore extends KeyStoreBase implements KeyStore {
	...
	async genKeyMaterial(cfg?: Partial<Config>): Promise<KeyMaterial> {
		...
		const encryptionKeyPair = await ecc.genKeyPair(
			mergedCfg.curve,
			KeyUse.Exchange
		);
	...
	async exportPrivateKeyMaterial(
		keyMaterial: KeyMaterial,
		cfg?: Partial<Config>
	): Promise<PrivateKeyMaterial> {
		const mergedCfg = config.merge(this.cfg, cfg);
		const encryptionPrivateKeyPem = await ecc.exportPrivateKeyPem(
			keyMaterial.encryptionKeyPair.privateKey as PrivateKey
		);
		const apiPrivateKeyPem = await ecc.exportPrivateKeyPem(
	...
	async escrowKeyMaterial(
		keyMaterial: KeyMaterial,
		passphrase: string,
		cfg?: Partial<Config>
	): Promise<EscrowedKeyMaterial> {
		const mergedCfg = config.merge(this.cfg, cfg);
		const salt = utils.randomBuf(DEFAULT_SALT_LENGTH);

		// Get the public key pems from the key material
		const encryptionPublicKeyPem = await ecc.exportPublicKeyPem(
			keyMaterial.encryptionKeyPair.publicKey as PublicKey
		);
		const apiPublicKeyPem = await ecc.exportPublicKeyPem(
			keyMaterial.apiKeyPair.publicKey as PublicKey
		);
	...
	async recoverKeyMaterial(
		escrowedKeyMaterial: EscrowedKeyMaterial,
		passphrase: string,
		cfg?: Partial<Config>
	): Promise<PrivateKeyMaterial> {
		const mergedCfg = config.merge(this.cfg, cfg);
		...
		const privateKeyMaterialString = await aes.decrypt(
			escrowedKeyMaterial.encryptedPrivateKeyMaterial,
			key
		);
		const privateKeyMaterial = JSON.parse(privateKeyMaterialString);
