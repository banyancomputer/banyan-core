...
import { CachedKeyMaterial, Config, PrivateKeyMaterial } from '../types';
	...
	async retrieveCachedPrivateKeyMaterial(
		...
		return idb.getBlob(sessionId, this.store).then((blob) => {
			...
			return blob.text().then((cachedKeyMaterialStr) => {
				const cachedKeyMaterial: CachedKeyMaterial =
					JSON.parse(cachedKeyMaterialStr);
				...
				return pbkdf2
					...
					.then((key) =>
						aes.decrypt(cachedKeyMaterial.encryptedPrivateKeyMaterial, key)
					)
					.then((privateKeyMaterialStr) => JSON.parse(privateKeyMaterialStr))
					.catch((err) => {
