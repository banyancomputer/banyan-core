...
pub async fn update_status(conn: &mut DatabaseConnection, payment_intent: &stripe::PaymentIntent) -> Result<(), StripeWebhookError> {
    ...
    invoice.update_intent_status(&mut *conn, intent_status).await?;
