            ...
            EmailTransport::Stub(transport) => {
                ...
                tracing::info!(
                    ...
                    std::str::from_utf8(&message.formatted()).map_err(|e| {
                        CoreError::default_error(&format!(
                            "CoreError could not decode email: {}",
                            e
                        ))
                    })?
                );
...
fn parse_smtp_url(url: &str) -> Result<(String, u16, Credentials), CoreError> {
    ...
    let host_port = parts
        .next()
        .ok_or(CoreError::default_error("missing host:port"))?;
    let creds = parts
        .next()
        .ok_or(CoreError::default_error("missing username:password"))?;

    let mut host_parts = host_port.split(':');
    let host = host_parts
        .next()
        .ok_or(CoreError::default_error("missing host"))?
        .to_string();
    let port: u16 = host_parts
        .next()
        .ok_or(CoreError::default_error("missing port"))?
        .parse()
        .map_err(|e| CoreError::default_error(&format!("Error parsing port: {}", e)))?;

    let mut creds_parts = creds.split(':');
    let username = creds_parts
        .next()
        .ok_or(CoreError::default_error("missing username"))?
        .to_string();
    let password = creds_parts
        .next()
        .ok_or(CoreError::default_error("missing password"))?
        .to_string();
