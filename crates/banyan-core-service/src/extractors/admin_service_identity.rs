...
use crate::app::{AdminServiceName, AdminServiceVerificationKey};

pub struct AdminServiceIdentity;

#[async_trait]
impl<S> FromRequestParts<S> for AdminServiceIdentity
where
    AdminServiceName: FromRef<S>,
    AdminServiceVerificationKey: FromRef<S>,
    S: Send + Sync,
{
    type Rejection = AdminServiceIdentityError;
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let admin_service_verification_key = AdminServiceVerificationKey::from_ref(state);

        let verification_options = VerificationOptions {
            accept_future: false,
            // TODO: this might not be a quite right, but it's probably fine for now
            allowed_audiences: Some(HashSet::from_strings(&[
                AdminServiceName::from_ref(state).to_string()
            ])),
        ...
        let claims = admin_service_verification_key
...
pub enum AdminServiceIdentityError {
...
impl IntoResponse for AdminServiceIdentityError {
    fn into_response(self) -> Response {
        use AdminServiceIdentityError::*;
