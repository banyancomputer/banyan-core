...
pub async fn create(
    ...
    if let Err(errors) = new_bucket.validate() {
        CoreError::generic_error(
            ...
            &format!("invalid bucket creation request: {errors}"),
            None,
        )
        .into_response()
    } else {
...
pub async fn read_all(api_token: ApiToken, mut db_conn: DbConn) -> Response {
    ...
    match db::read_all_buckets(&account_id, &mut db_conn).await {
        ...
        Err(err) => CoreError::generic_error(
            ...
            Some(&format!("failed to read all buckets: {err}")),
        )
        .into_response(),
    }
}
...
pub async fn read(
    ...
    match db::read_bucket(&account_id, &bucket_id, &mut db_conn).await {
        ...
        Err(err) => CoreError::generic_error(
            StatusCode::INTERNAL_SERVER_ERROR,
            "backend service issue",
            Some(&format!("unable to read bucket: {err}")),
        )
        .into_response(),
    }
}
...
pub async fn delete(
    ...
    if let Err(err) = db::delete_bucket(&account_id, &bucket_id, &mut db_conn).await {
        CoreError::generic_error(
            StatusCode::INTERNAL_SERVER_ERROR,
            "backend service issue",
            Some(&format!("unable to delete bucket: {err}")),
        )
        .into_response()
    } else {
    ...
    match db::read_bucket_data_usage(&bucket_id, &mut db_conn).await {
        ...
        Err(err) => match err {
            sqlx::Error::RowNotFound => {
                CoreError::sqlx_error(err, "read", "bucket").into_response()
            }
            _ => CoreError::generic_error(
                StatusCode::INTERNAL_SERVER_ERROR,
                "backend service issue",
                Some(&format!("unable to read bucket usage: {err}")),
            )
            .into_response(),
        },
    }
...
pub async fn get_total_usage(api_token: ApiToken, mut db_conn: DbConn) -> Response {
    match db::read_total_data_usage(&api_token.subject, &mut db_conn).await {
        ...
        Err(err) => match err {
            sqlx::Error::RowNotFound => {
                CoreError::sqlx_error(err, "read", "account").into_response()
            }
            _ => CoreError::generic_error(
                StatusCode::INTERNAL_SERVER_ERROR,
                "backend service issue",
                Some(&format!("unable to read account usage: {err}")),
            )
            .into_response(),
        },
    }
